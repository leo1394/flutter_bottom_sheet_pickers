...
`flutter_bottom_sheet_pickers` 是一个面向 Flutter 的底部弹窗选择器集合，当前支持单选、多选、搜索、分页懒加载、最多三级级联选择、日期、日期范围和年月选择。
...
语言: 中文 | [English](https://github.com/leo1394/flutter_bottom_sheet_pickers/blob/master/README.md)
...
| `BottomSheetPickers.dateRange(context)` | 日期范围选择 | `Future<DateTimeRange?>` |
| `BottomSheetPickers.yearMonth(context)` | 年月或年月范围选择 | `Future<YearMonth?>` / `Future<YearMonthRange?>` |
...
`firstDate` 和 `lastDate` 是可选日期的闭区间。日历头部的上一月、下一月、上一年、下一年按钮会根据这个范围自动显示或隐藏。
...
主标题和返回值始终使用公历日期；指定 `CalendarType` 后，day cell 下方会显示该历法的小数字或月份提示。
...
未传入初始范围时，开始年月默认显示当前年月，截止年月为空；选择截止年月时，候选项会从已选开始年月起算。
...
- 日期选择 confirm 返回 `DateTime?`
- 日期范围选择 confirm 返回 `DateTimeRange?`
- 年月选择 confirm 返回 `YearMonth?`
- 年月范围选择 confirm 返回 `YearMonthRange?`
